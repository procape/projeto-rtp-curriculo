...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function preload() {
        try {
            ...
            const cpfFromServerUrl = `${API_BASE}/colaboradores/me`
            try {
                const resCol = await fetch(cpfFromServerUrl, { method: 'GET', mode: 'cors', headers: { 'Authorization': `Bearer ${token}` } })
                if (!resCol.ok) {
                    const text = await resCol.text()
                    console.warn('colaboradores/me returned', resCol.status, resCol.statusText, text)
                } else {
                    const col = await resCol.json()
                    document.getElementById('nome_completo').value = col.nome || ''
                    document.getElementById('email').value = col.email_pessoal || ''
                    document.getElementById('telefone').value = col.telefone || ''
                    document.getElementById('cep').value = col.cep || ''
                    document.getElementById('logradouro').value = col.logradouro || ''
                    document.getElementById('numero').value = col.numero || ''
                    if (document.getElementById('complemento')) document.getElementById('complemento').value = col.complemento || ''
                    document.getElementById('bairro').value = col.bairro || ''
                    document.getElementById('cidade').value = col.cidade || ''
                    document.getElementById('estado').value = col.estado || ''
                }
            } catch (errCol) {
                console.warn('Erro ao buscar colaborador por user_id', errCol)
            }

            // then try to preload existing curriculo if present
            const res = await fetch(`${API_BASE}/curriculo/${user_id}`, { method: 'GET', mode: 'cors', headers: { 'Authorization': `Bearer ${token}` } })
            if (!res.ok) {
                const text = await res.text()
                console.warn('curriculo GET returned', res.status, res.statusText, text)
            }
            const lista = await res.json()
            if (Array.isArray(lista) && lista.length > 0) {
                const curr = lista[0]
                document.getElementById('nome_completo').value = curr.nome_completo || document.getElementById('nome_completo').value || ''
                document.getElementById('email').value = curr.email || document.getElementById('email').value || ''
                document.getElementById('telefone').value = curr.telefone || document.getElementById('telefone').value || ''
                ...
                document.getElementById('bairro').value = curr.bairro || document.getElementById('bairro').value || ''
                document.getElementById('cidade').value = curr.cidade || document.getElementById('cidade').value || ''
